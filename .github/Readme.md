...
OTFCareKit™ is an open source software framework for creating apps that help people better understand and manage their health. The framework provides modules that can be used out of the box, or extended and customized for more targeted use cases. It is composed of three SPM packages which can each be imported separately.

* **OTFCareKit:** This is the best place to start building your app. OTFCareKit provides view controllers that tie OTFCareKitUI and OTFCareKitStore together. The view controllers leverage Combine to provide synchronization between the store and the views.

* **OTFCareKitUI:** Provides the views used across the framework. The views are subclasses of UIView that are open and extensible. Properties within the views are public allowing for full control over the content.

* **OTFCareKitStore:** Provides a Core Data solution for storing patient data. It also provides the ability to use a custom store, such as a third party database or API.
...
* [OTFCareKit](#carekit)
    ...
    * [SwiftUI](#swiftui)
* [OTFCareKitUI](#carekitui)
    * [Tasks](#tasks)
    * [Charts](#charts)
    * [Contacts](#contacts)
    * [Styling](#styling)
    * [SwiftUI](#carekitui-swiftui)
* [OTFCareKitStore](#carekitstore)
    * [Store](#store)
    * [Schema](#schema)
...
The primary OTFCareKit framework codebase supports iOS and requires Xcode 12.0 or newer. The OTFCareKit framework has a Base SDK version of 13.0.
...
OTFCareKit can be installed via Cocoapods. To be able to install OTFCareKit via Cocoapods you should specify the podspec source as `source 'https://github.com/HippocratesTech/OTFCocoapodSpecs'` at the beginning of the pod file. Here is what you need to include in your `Podfile` to be able to install OTFCareKit.
...
The included catalog app demonstrates the different modules that are available in OTFCareKit: [OCKCatalog](OCKCatalog)
...
The included sample app demonstrates a fully constructed OTFCareKit app: [OCKSample](OCKSample)

![ocksample](https://user-images.githubusercontent.com/51756298/69107801-7586eb00-0a27-11ea-8aa2-eca687602c76.gif)

# OTFCareKit <a name="carekit"></a>

OTFCareKit is the overarching package that provides view controllers to tie OTFCareKitUI and OTFCareKitStore together. When importing OTFCareKit, OTFCareKitUI and OTFCareKitStore will be imported under the hood.

### List view controllers <a name="list-view-controllers"></a>

OTFCareKit offers full screen view controllers for convenience. The view controllers query for and display data from a store, and stay synchronized with the data.
...
Similar to our UIKit API, there are two types of views - views in OTFCareKitUI that can be initialized with static properties, and views in OTFCareKit that are dynamic and update when data in the store changes.

To create a view in OTFCareKit that is synchronized with the store:
...
Data from the controller will be automatically mapped to the view. When the data in the controller changes, the view will update itself. You may want to customize the way in which data is mapped from the controller to the view. To do so, use this initializer on the OTFCareKit view:

```swift
OTFCareKit.InstructionsTaskView(controller: controller) { configuration in

// Create an instance of a OTFCareKitUI.InstructionsTaskView to render. Use the `configuration` for the default values to map to the view.
OTFCareKitUI.InstructionsTaskView(title: Text("Custom Title"),    // Inject custom text into the title here.
...
This initializer requires the initialization of a static SwiftUI view from OTFCareKitUI. That opens up the customization points that are available in OTFCareKitUI. See [SwiftUI in OTFCareKitUI](#carekitui-swiftui)

You can also create your own SwiftUI views that are synchronized with the store. OTFCareKit controllers are compatible with SwiftUI and can help update the view when data in the store changes:
...
# OTFCareKitUI <a name="carekitui"></a>

OTFCareKitUI provides cards to represent tasks, charts, and contacts. There are multiple provided styles for each category of card.
...
# OTFCareKitStore <a name="carekitstore"></a>

The OTFCareKitStore package defines the `OCKStoreProtocol` that OTFCareKit uses to talk to data stores, and a concrete implementation that leverages CoreData, called `OCKStore`.
It also contains definitions of most of the core structures and data types that OTFCareKit relies on, such `OCKAnyTask`, `OCKTaskQuery`, and `OCKSchedule`.

### Store <a name="store"></a>

The `OCKStore` class is an append-only, versioned store packaged with OTFCareKit. It is implemented on top of CoreData and provides fast, secure, on-device storage.
`OCKStore` was designed to integrate with OTFCareKit's synchronized view controllers, but can be used in isolation as well.
...
This graphic visualizes how results are retrieved when querying versioned objects in OTFCareKit. Note how a query over a date range returns the version of the object valid in that date range.  
...
OTFCareKitStore defines six high level entities as illustrated in this diagram:
...
It is important to note that tasks, contacts, and care plans can exist *without* a parent entity. Many OTFCareKit apps are targeted to well defined use cases, and it can often be expedient to simply create tasks and contacts without defining a patient or care plan.

### Scheduling <a name="scheduling"></a>

The scheduling tools provided in OTFCareKit allow very precise and customizable scheduling of tasks. Each instance of an `OCKSchedule` is created by composing one or more
...
* `text`: By default, OTFCareKit view controllers will prompt users to perform tasks using clock time (e.g. "8:00PM"). If you provide a `text` property, then the text will be used to prompt the user instead ("Before bed").

* `duration`: If you provide a duration, OTFCareKit will prompt the user to perform the scheduled task within a window (e.g. "8:00 - 10:00 PM"). The duration can also be set to `.allDay` if you do not wish to specify any time in particular.

* `targetValues`: Target values are used by OTFCareKit to determine if a user has completed a specific task or not. See `OCKAdherenceAggregator` for more details.


### Custom Stores and Types <a name="custom-store-and-types"></a>

The `OCKStore` class provided with OTFCareKit is a fast, secure, on-device store and will serve most use cases well. That said, we recognize it may not fully meet the needs of
all our developers, so OTFCareKit also allows you to write your own store. For example, you could write a wrapper around a web server, or even a simple JSON file. Any class that
conforms to the `OCKStoreProtocol` can be used in place of the default store.

Writing a OTFCareKit store adapter requires defining the entities that will live in your store and implementing asynchronous **Create**, **Read**, **Update**, and **Delete** methods for each.
...
Using the four basic CRUD methods you supply, OTFCareKit is able to use protocol extensions to imbue your store with extra functionality. For example, a store that implements
...
OTFCareKit and ResearchKit are sister frameworks and are designed to integrate well with one another.

When integrating a ResearchKit into your OTFCareKit app, there are a series of steps you will need to follow.
...
4. Get the survey result and save it to OTFCareKit's store

Here is an example demonstrating how to prompt the user to rate their pain on a scale of 1-10.
Keep in mind as you're reading the code below that OTFCareKit and ResearchKit both use the term "task", but that they are distinct.
...
class SurveyViewController: OCKInstructionsTaskViewController, ORKTaskViewControllerDelegate {
    ...
    func taskViewController(_ taskViewController: ORKTaskViewController, didFinishWith reason: ORKTaskViewControllerFinishReason, error: Error?) {        
        ...
        // 4b. Save the result into OTFCareKit's store
        controller.appendOutcomeValue(withType: answer, at: IndexPath(item: 0, section: 0), completion: nil)
    }
}
```

Once you have defined this view controller, you can add it into your app as you would any other OTFCareKit view controller!
...
GitHub is our primary forum for OTFCareKit. Feel free to open up issues about questions, problems, or ideas.
